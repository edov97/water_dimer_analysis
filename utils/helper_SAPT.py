###
# Psi4numpy layers
# Moved here for /import
# Modified helper_SAPT to introduce new modified orbitals
...
class helper_SAPT(object):
    ...
    def __init__(self, dimer, memory=37, algorithm='MO', reference='RHF'):
        ...
        ##NOTE:Added to evaluate hamiltonian matrices in MO
        TA = np.asarray(psi4.core.MintsHelper(self.wfnA.basisset()).ao_kinetic())
        TB = np.asarray(psi4.core.MintsHelper(self.wfnB.basisset()).ao_kinetic())
        self.HA = TA + self.V_A
        self.HB = TB + self.V_B
    ...
    # NOTE: Added to introduce new modified orbital coefficients
    # TODO: Modify V tensor and S with dependence on modified C values
    def set_orbitals(self, ca, cb):
        self.C_A = np.asarray(ca)
        self.C_B = np.asarray(cb)
    ...
    def v(self, string):
        ...
        V = oe.contract('pA,pqrs->Aqrs', self.orbitals[string[0]], self.I)
    ...
    def vt(self, string):
        ...
        V += oe.contract('ik,jl->ijkl', V_B, S_B)

        # Nuclear
        V += oe.contract('ik,jl->ijkl', S_A, S_B) * self.vt_nuc_rep
    ...
    def chf(self, monomer, ind=False):
        ...
        if monomer == 'A':
            ...
            w_n = 2 * oe.contract('saba->bs', self.v('saba'))
        ...
        if monomer == 'B':
            w_n = 2 * oe.contract('rbab->ar', self.v('rbab'))
        ...
        A_ovOV = oe.contract('vOoV->ovOV', v_vOoV + v_vVoO.swapaxes(1, 3))
        ...
        if ind:
            ...
            e20_ind = 2 * oe.contract('vo,ov->', t, w_n)
            # print('####### Extra #######')
            # print(t.shape)
            # print(w_n.shape)
            return (t, e20_ind)
    ...
    def h_tot(self, monomer, string):
        if len(string) != 2:
            psi4.core.clean()
            raise Exception('Hamiltonian: string %s does not have 2 elements.' % string)
        
        if monomer == 'A':
            return (self.orbitals[string[0]].T).dot(self.HA).dot(self.orbitals[string[1]])
        elif monomer == 'B':
            return (self.orbitals[string[0]].T).dot(self.HB).dot(self.orbitals[string[1]])

    def fock(self, monomer, string):
        if len(string) != 2:
            psi4.core.clean()
            raise Exception('Fock: string %s does not have 2 elements.' % string)
        sapt_I = self.I.swapaxes(1,2)
        
        if monomer == 'A':
            DA = oe.contract('pi,qi->pq', self.Co_A, self.Co_A)
            JA = oe.contract('pqrs,rs->pq', sapt_I, DA)
            KA = oe.contract('prqs,rs->pq', sapt_I, DA)
            FA = self.HA + 2*JA - KA
            return (self.orbitals[string[0]].T).dot(FA).dot(self.orbitals[string[1]])
        
        elif monomer == 'B':
            DB = oe.contract('pi,qi->pq', self.Co_B, self.Co_B)
            JB = oe.contract('pqrs,rs->pq', sapt_I, DB)
            KB = oe.contract('prqs,rs->pq', sapt_I, DB)     
            FB = self.HB + 2*JB - KB
            return (self.orbitals[string[0]].T).dot(FB).dot(self.orbitals[string[1]])
