...
def do_nopb_itr(dimer: psi4.geometry, 
    ...
    for i in range(1, maxiter + 1):
        ...
        # --- print Electrostatic A potentials for debugging: ---
        # -- A potentials --
        #print('sq VA: \n', VA)        
        #print('sq JA: \n', JA_prev)
        #print('sq WA tot: \n', WA_elst_ao_prev)
        ...
        if not omega_elst_only: 
                    ...
                    if non_s2:
                        ...
                        if lm_bool:
                            lmA_mo_prev, lmB_mo_prev = form_lm_terms_s2(h_sapt,
                                                                        ca= CA_prev,
                                                                        cb=CB_prev)
                            WA_exch_mo_prev_nonlm = WA_exch_mo_prev 
                            WB_exch_mo_prev_nonlm = WB_exch_mo_prev 
                            WA_exch_mo_prev= WA_exch_mo_prev + lmA_mo_prev
                            WB_exch_mo_prev = WB_exch_mo_prev + lmB_mo_prev
                    else:
                        # All blocks (S2)
            ...
            # MO --> AO
            # Omega Exchange (AO)
            WA_exch_ao_prev_nonlm = S.dot(CB_prev).dot(WA_exch_mo_prev_nonlm).dot(CB_prev.T).dot(S)
            WB_exch_ao_prev_nonlm = S.dot(CA_prev).dot(WB_exch_mo_prev_nonlm).dot(CA_prev.T).dot(S)
            print('sq LM potential for A in MO:\n', lmB_mo_prev)
            print('sq LM potential for B in MO:\n', lmA_mo_prev)
            lmA_ao_prev = S.dot(CB_prev).dot(lmA_mo_prev).dot(CB_prev.T).dot(S)
            lmB_ao_prev = S.dot(CA_prev).dot(lmB_mo_prev).dot(CA_prev.T).dot(S)
            print('sq LM potential for A in AO:\n', lmB_ao_prev)
            print('sq LM potential for B in AO:\n', lmA_ao_prev)

            # --- print the exchannge potential without LM: ---
            #print('sq A exchange potential only:\n', WB_exch_ao_prev_nonlm)
            #print('sq B exchange potential only:\n', WA_exch_ao_prev_nonlm)

            # --- print the exchannge potential with LM: ---
            ...
            #print('sq A exchange potential with LM:\n', WB_exch_ao_prev)
            #print('sq B exchange potential with LM:\n', WA_exch_ao_prev)
        ...
        if i == 1:
            ...
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ## --- print WA without LM for debugging ---
        WA_tot_ao_prev_nonlm = WA_elst_ao_prev + WA_exch_ao_prev_nonlm 
        WB_tot_ao_prev_nonlm = WB_elst_ao_prev + WB_exch_ao_prev_nonlm
        print('####### A Elst+Exch SQ ########:\n', WB_tot_ao_prev_nonlm)
        print('####### B Elst+Exch SQ ########:\n', WA_tot_ao_prev_nonlm)
...
"""
        ...
        if (abs(dE_int_lm) < E_conv) and i>1 and not converged: 
            ...
            return CA_CONV, CB_CONV, E_int_w_lm, dEA, dEB, E1, del_FA, del_FB
